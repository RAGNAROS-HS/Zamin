...
def initialize_pop(population_size):
    x = []
    dims = 1343 #calculated from NN topology
...
def selection(population, pressure):
    tournament = random.sample(population, k=3) #need to remember about parameter tuning
...
def nn_training_loop(epoch, batch):
    ...
    plt.savefig('nn_test_scatter.png', dpi=300, bbox_inches='tight')
    plt.show()
